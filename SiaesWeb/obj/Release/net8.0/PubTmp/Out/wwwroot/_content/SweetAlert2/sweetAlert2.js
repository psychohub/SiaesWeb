...
;// CONCATENATED MODULE: ./src/ts/ColorScheme.ts
let ColorScheme = /*#__PURE__*/function (ColorScheme) {
  ColorScheme[ColorScheme["NoPreference"] = 0] = "NoPreference";
  ColorScheme[ColorScheme["Light"] = 1] = "Light";
  ColorScheme[ColorScheme["Dark"] = 2] = "Dark";
  return ColorScheme;
}({});
;// CONCATENATED MODULE: ./src/ts/SweetAlertTheme.ts
let SweetAlertTheme = /*#__PURE__*/function (SweetAlertTheme) {
  SweetAlertTheme[SweetAlertTheme["Default"] = 0] = "Default";
  SweetAlertTheme[SweetAlertTheme["Dark"] = 1] = "Dark";
  SweetAlertTheme[SweetAlertTheme["Minimal"] = 2] = "Minimal";
  SweetAlertTheme[SweetAlertTheme["Borderless"] = 3] = "Borderless";
  SweetAlertTheme[SweetAlertTheme["Bootstrap4"] = 4] = "Bootstrap4";
  SweetAlertTheme[SweetAlertTheme["MaterialUI"] = 5] = "MaterialUI";
  SweetAlertTheme[SweetAlertTheme["WordpressAdmin"] = 6] = "WordpressAdmin";
  SweetAlertTheme[SweetAlertTheme["Bulma"] = 7] = "Bulma";
  return SweetAlertTheme;
}({});
;// CONCATENATED MODULE: ./src/ts/SweetAlert.ts



const namespace = "SweetAlert2";
window.Swal = (sweetalert2_all_default());
disableProtestware();
function getEnumNumber(enumString) {
  switch (enumString) {
    case (sweetalert2_all_default()).DismissReason.cancel:
      return 0;
    case (sweetalert2_all_default()).DismissReason.backdrop:
      return 1;
    case (sweetalert2_all_default()).DismissReason.close:
      return 2;
    case (sweetalert2_all_default()).DismissReason.esc:
      return 3;
    case (sweetalert2_all_default()).DismissReason.timer:
      return 4;
  }
  return undefined;
}
function getEnumString(enumNumber) {
  switch (enumNumber) {
    case 0:
      return (sweetalert2_all_default()).DismissReason.cancel;
    case 1:
      return (sweetalert2_all_default()).DismissReason.backdrop;
    case 2:
      return (sweetalert2_all_default()).DismissReason.close;
    case 3:
      return (sweetalert2_all_default()).DismissReason.esc;
    case 4:
      return (sweetalert2_all_default()).DismissReason.timer;
  }
  return undefined;
}
function getStringVersion(input) {
  if (input instanceof Object) {
    return JSON.stringify(input);
  }
  return String(input);
}
function dispatchFireResult(requestId, result) {
...
function dispatchPreConfirm(requestId, inputValue) {
  return DotNet.invokeMethodAsync(namespace, "ReceivePreConfirmInput", requestId, getStringVersion(inputValue));
}
function dispatchPreDeny(requestId, inputValue) {
  return DotNet.invokeMethodAsync(namespace, "ReceivePreDenyInput", requestId, getStringVersion(inputValue));
}
function dispatchInputValidator(requestId, inputValue) {
  return DotNet.invokeMethodAsync(namespace, "ReceiveInputValidatorInput", requestId, inputValue);
}
function dispatchDidOpen(requestId) {
  DotNet.invokeMethodAsync(namespace, "ReceiveDidOpenInput", requestId);
}
function dispatchWillClose(requestId) {
  DotNet.invokeMethodAsync(namespace, "ReceiveWillCloseInput", requestId);
}
function dispatchDidRender(requestId) {
  DotNet.invokeMethodAsync(namespace, "ReceiveDidRenderInput", requestId);
}
function dispatchWillOpen(requestId) {
  DotNet.invokeMethodAsync(namespace, "ReceiveWillOpenInput", requestId);
}
function dispatchDidClose(requestId) {
  DotNet.invokeMethodAsync(namespace, "ReceiveDidCloseInput", requestId);
}
function dispatchDidDestroy(requestId) {
...
function getSwalSettingsFromPoco(settings, requestId) {
  const swalSettings = cleanSettings(settings);
  function processPreConfirmDenyResult(value) {
    if (value === null) {
      return undefined;
    }
    if (value === "false") {
  ...
  if (settings.preConfirm) {
    swalSettings.preConfirm = inputValue => dispatchPreConfirm(requestId, inputValue).then(processPreConfirmDenyResult);
  } else {
    delete swalSettings.preConfirm;
  }
  if (settings.preDeny) {
    swalSettings.preDeny = inputValue => dispatchPreDeny(requestId, inputValue).then(processPreConfirmDenyResult);
  } else {
...
function getFileNameByTheme(theme) {
  switch (theme) {
...
function setTheme(theme, colorSchemeThemes) {
  ...
  if (theme !== SweetAlertTheme.Default && themeShouldBeSet) {
    ...
    styleTag.id = tagId;
    styleTag.href = `_content/SweetAlert2/${getFileNameByTheme(theme)}`;
    styleTag.setAttribute("data-theme-number", String(theme));
    head.appendChild(styleTag);
  }
  colorSchemeMap.forEach((theme, colorScheme) => {
    const schemeTagId = `sweetalert2-scheme-link-${colorScheme}`;
    ...
    schemeTag.href = `_content/SweetAlert2/${getFileNameByTheme(theme)}`;
...
window.SweetAlert2 = window.SweetAlert2 ?? {};
const razorSwal = window.SweetAlert2;
razorSwal.SendThemesToJS = (theme, colorSchemeThemes) => {
  setTheme(theme, colorSchemeThemes);
};
razorSwal.Fire = (requestId, title, message, icon) => {
  const params = [title ?? undefined, message ?? undefined, icon ?? undefined];
  sweetalert2_all_default().fire(params[0], params[1], params[2]).then(result => {
    dispatchFireResult(requestId, result);
  });
};
razorSwal.FireSettings = (requestId, settingsPoco) => {
  const swalSettings = getSwalSettingsFromPoco(settingsPoco, requestId);
  sweetalert2_all_default().fire(swalSettings).then(result => {
    dispatchFireResult(requestId, result);
  });
};
razorSwal.IsVisible = () => {
  return !!sweetalert2_all_default().isVisible();
};
razorSwal.Update = (requestId, settingsPoco) => {
  const swalSettings = getSwalSettingsFromPoco(settingsPoco, requestId);
  sweetalert2_all_default().update(swalSettings);
};
razorSwal.CloseResult = result => {
...
razorSwal.EnableButtons = () => {
  sweetalert2_all_default().enableButtons();
};
razorSwal.DisableButtons = () => {
  sweetalert2_all_default().disableButtons();
};
razorSwal.ShowLoading = () => {
  sweetalert2_all_default().showLoading();
};
razorSwal.HideLoading = () => {
  sweetalert2_all_default().hideLoading();
};
razorSwal.IsLoading = () => {
  return sweetalert2_all_default().isLoading();
};
razorSwal.ClickConfirm = () => {
  sweetalert2_all_default().clickConfirm();
};
razorSwal.ClickDeny = () => {
  sweetalert2_all_default().clickDeny();
};
razorSwal.ClickCancel = () => {
  sweetalert2_all_default().clickCancel();
};
razorSwal.ShowValidationMessage = validationMessage => {
  sweetalert2_all_default().showValidationMessage(validationMessage);
};
razorSwal.ResetValidationMessage = () => {
  sweetalert2_all_default().resetValidationMessage();
};
razorSwal.DisableInput = () => {
  sweetalert2_all_default().disableInput();
};
razorSwal.EnableInput = () => {
  sweetalert2_all_default().enableInput();
};
razorSwal.GetTimerLeft = () => {
  return sweetalert2_all_default().getTimerLeft();
};
razorSwal.StopTimer = () => {
  return sweetalert2_all_default().stopTimer();
};
razorSwal.ResumeTimer = () => {
  return sweetalert2_all_default().resumeTimer();
};
razorSwal.ToggleTimer = () => {
  return sweetalert2_all_default().toggleTimer();
};
razorSwal.IsTimerRunning = () => {
  return sweetalert2_all_default().isTimerRunning();
};
razorSwal.IncreaseTimer = n => {
  return sweetalert2_all_default().increaseTimer(n);
};
razorSwal.IsValidParameter = paramName => {
  return sweetalert2_all_default().isValidParameter(paramName);
};
razorSwal.IsUpdatableParameter = paramName => {
  return sweetalert2_all_default().isUpdatableParameter(paramName);
};
function disableProtestware() {
    ...
    mutations.forEach(childListMutation => {
      childListMutation.addedNodes.forEach(addedNode => {
        if (addedNode.nodeName.toLowerCase() === "audio") {
          const audioNode = addedNode;
          if (audioNode.src === "https://flag-gimn.ru/wp-content/uploads/2021/09/Ukraina.mp3") {
  ...
  protestwareObserver.observe(document.body, {
    childList: true
  });
}
})();
