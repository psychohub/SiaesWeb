...
{
    public class TUbicacionService : IServicioTUbicacion
    {
        private readonly HttpClient? _cliente;
        private readonly ILocalStorageService? _localStorage;
        private readonly AuthenticationStateProvider _estadoProveedorAutenticacion;

        public TUbicacionService(HttpClient cliente,
        ...
        public async Task<IEnumerable<TUbicacionDTO>> GetUbicacionAsync()
        {
            ...
            var response = await _cliente.GetAsync($"{Inicializar.UrlBaseApi}api/siaes/registrodiario/modalidad");
            response.EnsureSuccessStatusCode();

            var contentTemp = await response.Content.ReadAsStringAsync();
            var resultado = JsonConvert.DeserializeObject<IEnumerable<TUbicacionDTO>>(contentTemp);
            return resultado;
